...
module.exports = function (suite) {
  ...
  suite.ok("tracked items appear", !!row("Arrow"));
  suite.ok("an item-backed row knows its item", !!row("Arrow").item);
  suite.ok("arrows are not also a standalone resource",
    !character.resources.some(r => r.name === "Arrow"));

  suite.section("a stack counts itself");
  suite.is("the row reads the quantity", row("Arrow").current, item("Arrow").qty);
  adjustResourceRow(findResourceRow(character, row("Arrow").key), -5);
  suite.is("the stepper writes to the quantity", item("Arrow").qty, 55);
  suite.is("and the row follows", row("Arrow").current, 55);
  suite.near("weight follows the count", calculateCarriedWeight(character).sources
    .find(s => s.label === "Arrow").value, 2.75, 0.01);
  item("Arrow").qty = 60;

  suite.section("a container holds a count");
  suite.ok("the quiver is a container", isContainer(item("Quiver")));
  suite.ok("the arrow stack is not", !isContainer(item("Arrow")));
  ...
  suite.is("the pack is untouched", item("Arrow").qty, 60);

  suite.section("refilling conserves arrows");
  item("Quiver").resource.loaded = 0;
  const total = item("Arrow").qty;
  ...
  suite.is("the pack is down by exactly that", item("Arrow").qty, total - 20);
  suite.is("nothing was created", row("Quiver").current + item("Arrow").qty, total);
  ...
  suite.is("leaving the pack empty", item("Arrow").qty, 0);
  ...
  item("Quiver").resource.refillFrom = "Arrow";
  item("Arrow").qty = 60;
  item("Quiver").resource.loaded = 20;

  suite.section("item-backed resources recharge like any other");
  item("Arrow").resource.recharge = { on: "LR", amount: "all" };
  item("Arrow").resource.max = 60;
  item("Arrow").qty = 2;
  applyRest("short");
  suite.is("not on a short rest", item("Arrow").qty, 2);
  applyRest("long");
  suite.is("restored on a long rest", item("Arrow").qty, 60);
  item("Arrow").resource.recharge = { on: "none", amount: "all" };
  item("Arrow").resource.max = 0;
  ...
  suite.ok("it just leaves the resource list", !resourceRows(character).some(r => r.name === "Arrow"));
  suite.ok("and is still in the bags", /Arrow/.test(app.renderInventoryTab()));
  arrows.resource = { max: 0, recharge: { on: "none", amount: "all" } };

  suite.section("a quiver still finds arrows that stopped being a resource");
  delete item("Arrow").resource;
  item("Arrow").qty = 40;
  item("Quiver").resource.loaded = 0;
  ...
  suite.is("taking them out of the stack", item("Arrow").qty, 20);
  suite.is("and into the quiver", row("Quiver").current, 20);
  item("Arrow").resource = { max: 0, recharge: { on: "none", amount: "all" } };
